...
class _ContractorLoginScreenState extends State<ContractorLoginScreen> {
  ...
  Future<void> _loginContractor() async {
    ...
    try {
      ...
      // 2. CHECK SECURITY: Get the Custom Claims (The "Contractor Badge")
      // forceRefresh: true is critical to ensure role updates are fetched
      IdTokenResult tokenResult =
          await userCredential.user!.getIdTokenResult(true);

      // 3. Verify the role
      if (tokenResult.claims?['role'] == 'contractor') {
        if (mounted) {
          // ✅ Success: Navigate to Contractor Dashboard
          Navigator.pushReplacementNamed(context, '/contractor_dashboard');
        }
      } else {
        // ⛔ Failed: User exists, but is NOT a contractor
        await FirebaseAuth.instance.signOut(); // Kick them out immediately
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('⛔ Access Denied: Authorized Contractors Only.'),
  ...
  Widget build(BuildContext context) {
    // 🎨 THEME COLORS (Consistent with FixCity brand)
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              // 1. ICON & TITLE
              const Icon(
                Icons.engineering, // Changed to Engineering icon
                size: 80,
                color: primaryColor,
              ),
                ...
                child: Column(
                  children: [
                    TextField(
                      ...
                      decoration: _inputDecoration("Contractor Email", Icons.email),
                    ),
                    const SizedBox(height: 20),
                    TextField(
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed: _isLoading ? null : _loginContractor,
